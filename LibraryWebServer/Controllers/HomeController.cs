  ...
  public class HomeController : Controller
  {
    ...
    private readonly ILogger<HomeController> _logger;
        ...
        public IActionResult CheckLogin(string name, int cardnum)
        {
            ...
            using (Team100LibraryContext db = new Team100LibraryContext())
            {
                var query =
        ...
        public ActionResult AllTitles()
        {
            using(Team100LibraryContext db = new Team100LibraryContext())
            {
                var query =
                    ...
                    join p in db.Patrons on j2.CardNum equals p.CardNum into list
                    from j3 in list.DefaultIfEmpty()
                    select new
                    {
                        ISBN = t.Isbn,
                        Title = t.Title,
        ...
        public ActionResult ListMyBooks()
        {
            using (Team100LibraryContext db = new Team100LibraryContext())
            {
                var query =
                    ...
                    join c in db.CheckedOut on j1.Serial equals c.Serial into chkout
                    from j2 in chkout.DefaultIfEmpty()
                    join p in db.Patrons on j2.CardNum equals p.CardNum where p.Name == user
                    select new
                    {
                        Title = t.Title,
                        Author = t.Author,
